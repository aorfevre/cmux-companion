...
import { acceptedReview } from './review.mjs';
import { planTarget } from './transitions.mjs';
import { requiresPlanReview } from './review-repairs.mjs';
import { actionView } from './action-view.mjs';
import { reviewRoundPhase } from './review-round.mjs';
...
export function goalView(goal) {
  return { ...actionView(goal), repositoryId: goal.repositoryId, id: goal.id, version: goal.version, generation: goal.generation, title: goal.description === undefined ? shortGoalTitle(goal.title) : goal.title, description: goal.description ?? goal.title, projectCode: goal.projectCode, plannerName: goal.plannerName, clarification: goal.clarification ?? null,
    planRevisionCount: goal.planRevisionCount ?? 0, planReviewEnabled: goal.planReviewEnabled ?? true, planReviewRequired: requiresPlanReview(goal), planReviewPending: requiresPlanReview(goal) && !acceptedReview(goal, planTarget(goal), 'plan'), baseBranch: goal.baseBranch, baseSha: goal.baseSha, startup: goal.startup ?? null,
    ...
    integrationHead: goal.integrationHead, integration: goal.integration ? { state: goal.integration.state, taskId: goal.integration.taskId, expectedHead: goal.integration.expectedHead, candidateSha: goal.integration.candidateSha, code: goal.integration.code ?? null } : null, integrationResults: goal.integrationResults ?? [], pr: goal.pr, mergeSync: goal.mergeSync ?? null, reviewRound: goal.reviewRound ? { ...goal.reviewRound, phase: reviewRoundPhase(goal.reviewRound) } : null, reviewRounds: (goal.reviewRounds ?? []).map(round => ({ ...round, phase: reviewRoundPhase(round) })), verification: goal.verification,
