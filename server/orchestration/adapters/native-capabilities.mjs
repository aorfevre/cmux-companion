 ...
 * @param {{ ccsBin: string; claudeBin: string }} options */
export async function probeNativeCapabilities({ ccsBin, claudeBin }) {
  try {
    const bin = executable(ccsBin), nativeBin = executable(claudeBin);
    const packagePath = resolve(dirname(bin), '..', 'package.json');
    requireValue(lstatSync(packagePath).size <= 1024 * 1024, 'Invalid CCS installation metadata', 'UNSUPPORTED_CAPABILITY');
    const packageStamp = fingerprint(packagePath);
    const metadata = JSON.parse(readFileSync(packagePath, 'utf8'));
    requireValue(metadata.name === '@kaitranntt/ccs' && CCS_VERSIONS.has(metadata.version)
    ...
    const version = versionResponse.value.stdout.trim().match(/^(\d+\.\d+\.\d+) \(Claude Code\)$/)?.[1];
    const help = helpResponse.value.stdout;
    requireValue(version && CLAUDE_VERSIONS.has(version) && FLAGS.every((flag) => help.includes(flag))
    ...
    const identity = Object.freeze({ bin, nativeBin, packagePath, nativeStamp, wrapperStamp, packageStamp });
    const assertCurrent = () => assertNativeInstallation(identity);
    assertCurrent();
    return Object.freeze({
      bin, nativeBin, identity, assertCurrent,
      evidence: Object.freeze({ claudeVersion: version, ccsVersion: String(metadata.version), permissionEnforcement: 'unverified' }),
      ...
      env: Object.freeze({ CCS_CLAUDE_PATH: nativeBin }),
 ...
 * @param {{bin:string; nativeBin:string; packagePath:string; nativeStamp:string; wrapperStamp:string; packageStamp:string}} identity */
