 ...
 * @param {{directory:string; installation:Awaited<ReturnType<typeof import('./native-capabilities.mjs').probeNativeCapabilities>>; engine:import('./ccs.mjs').Engine; env:NodeJS.ProcessEnv; cmux:{bin:string;env:NodeJS.ProcessEnv}; policy:import('../types.d.ts').BackgroundPolicy}} options
 * @param {Parameters<Parameters<typeof import('../create-runtime.mjs').createRuntime>[0]['createAgents']>[0]} context */
export function createNativeAgents({ directory, installation, engine, env, cmux, policy }, context) {
  installation.assertCurrent();
  const inputs = new NativeInputs({ installation, engine, capabilities: installation.capabilities, env, describe: context.describe });
