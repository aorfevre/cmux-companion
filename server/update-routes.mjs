...
export function registerUpdateRoutes(app, { control, token, maintenance, requestCheck = () => control.check() }) {
  ...
  app.get('/api/updater/updates', async () => ({ ...control.status(), blockers: maintenance?.blockers() ?? ['Companion workflow state is unavailable'], blockerDetails: maintenance?.diagnostics?.() ?? [] }));
