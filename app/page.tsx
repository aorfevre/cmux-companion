...
export default function Home() {
  useEffect(() => {
    ...
    if (query.get('view') === 'worktrees') location.replace('/orchestration');
  }, []);
  ...
  useEffect(() => { const back = () => { const query = new URLSearchParams(location.search); setView((query.get('view') || 'sessions') as View); setSelectedWorkspaceId(query.get('workspace')); setSelectedTerminalId(query.get('surface')); setDetailTab((query.get('tab') || 'terminal') as DetailTab); }; window.addEventListener('popstate', back); return () => window.removeEventListener('popstate', back); }, []);
  ...
  function changeView(next: View) { if (next === "settings") { location.assign("/settings"); return; } if (next === "worktrees") { location.assign("/orchestration"); return; } setSelectedWorkspaceId(null); setDocumentTarget(null); setActionId(null); setView(next); history.pushState(null, "", `/?view=${next}`); }
...
export function BottomNav({ view, onView }: { view: View; onView: (view: View) => void }) { return <AppNavigation active={view === 'inbox' ? 'inbox' : 'sessions'} onNavigate={onView} />; }
