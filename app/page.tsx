...
export default function Home() {
  ...
  async function sendPrompt(event: FormEvent) { event.preventDefault(); if (!selectedTerminal || (!draft.trim() && attachments.length === 0) || readOnly) return; const imageLines = attachments.map((image) => `- ${image.path}`); const text = [draft.trim(), imageLines.length ? `Attached image${imageLines.length > 1 ? "s" : ""}:\n${imageLines.join("\n")}` : ""].filter(Boolean).join("\n\n"); setSending(true); try { await api(`/api/terminals/${selectedTerminal.id}/input`, { method: "POST", body: JSON.stringify({ text, enter: true }) }); setDraft(""); clearAttachments(); setTimeout(loadTerminal, 200); } catch (error) { setNotice(error instanceof Error ? error.message : "Could not send input"); } finally { setSending(false); } }
  ...
  if (selectedWorkspace && selectedTerminal) return <WorkspaceDetail workspace={selectedWorkspace} terminal={selectedTerminal} repo={selectedRepo} tab={detailTab} terminalView={terminalView} screenError={screenError} draft={draft} attachments={attachments} sending={sending} readOnly={readOnly} onBack={closeWorkspaceView} onTab={setDetailTab} onTerminal={(id) => { setSelectedTerminalId(id); setTerminalView(null); clearAttachments(); }} onDraft={setDraft} onImage={addImage} onRemoveImage={removeImage} onSubmit={sendPrompt} onKey={sendKey} onReadOnly={() => changeReadOnly(!readOnly)} onRefresh={loadTerminal} onChanged={async () => { await loadBootstrap(); }} onNotice={setNotice} />;
...
function TerminalPanel(props: { workspace: Workspace; terminal: Terminal; terminalView: TerminalView | null; screenError: string; draft: string; attachments: ImageAttachment[]; sending: boolean; readOnly: boolean; onTerminal: (id: string) => void; onDraft: (value: string) => void; onImage: (file: File) => void; onRemoveImage: (path: string) => void; onSubmit: (event: FormEvent) => void; onKey: (key: string) => void; onReadOnly: () => void; onRefresh: () => void }) {
