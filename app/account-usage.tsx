...
export function AccountUsageView({ onBack }: { onBack: () => void }) {
  ...
  return <section className="account-usage-page">
    ...
    <div className="usage-providers">{usage?.providers.map((provider) => <ProviderSection provider={provider} key={provider.id} />)}</div>
...
function ProviderSection({ provider }: { provider: UsageProvider }) {
  return <section className="usage-provider"><header><div className={`provider-mark ${provider.id}`}>{provider.id === "claude" ? "C" : "O"}</div><div><h2>{provider.label}</h2><span>{provider.accounts.length} connected account{provider.accounts.length === 1 ? "" : "s"}</span></div></header>
    ...
    <div className="usage-account-list">{provider.accounts.map((account) => <AccountCard account={account} key={account.id} />)}</div>
  </section>;
}
...
function AccountCard({ account }: { account: UsageAccount }) {
  ...
  return <article className={`usage-account ${account.status}`}><header><div><strong>{account.email || account.label}</strong><span>{[account.plan, account.isDefault ? "default" : null, account.paused ? "paused" : null].filter(Boolean).join(" · ") || "CCS account"}</span></div><span className={`usage-status ${account.status}`}>{statusLabel(account.status)}</span></header>
    {(account.status === "reconnect" || account.status === "unavailable") && <p className="account-message">{account.message}</p>}
...
function statusLabel(status: UsageAccount["status"]) { return ({ ready: "Available", low: "Low", exhausted: "Exhausted", reconnect: "Reconnect", unavailable: "Unavailable" })[status]; }
