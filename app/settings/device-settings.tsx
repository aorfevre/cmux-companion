...
import { disablePush } from '../notification-preferences';
...
export function DeviceSettings() {
  ...
  return <section><h2>General</h2><p>Device preferences apply to this browser. Repository and agent settings are shared on the connected Mac.</p>
    ...
    <button onClick={() => { void (async () => {
      let cleanupError = '';
      try { await disablePush(); } catch (cause) { cleanupError = cause instanceof Error ? cause.message : 'Push cleanup failed.'; }
      await request('/api/auth/logout', { method: 'POST', body: '{}' });
      if (cleanupError) { setError(`Unpaired. ${cleanupError}`); setNotice('You are signed out. Re-pair to retry push cleanup, or revoke all subscriptions from another device.'); }
      else location.assign('/');
    })().catch(() => setError('Could not unpair. Try again.')); }}>Unpair this device</button>
