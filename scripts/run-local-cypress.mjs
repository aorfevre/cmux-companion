import { settingsSpecs } from './settings-cypress-specs.mjs';
...
const index = process.argv.indexOf('--spec'), suppliedSpec = index < 0 ? null : process.argv[index + 1];
if (index >= 0 && !suppliedSpec) throw new Error('--spec needs a spec path.');
if (settings && orchestration) throw new Error('Choose settings or orchestration fixtures, not both.');
const selections = settings ? settingsSpecs(suppliedSpec ?? undefined, { open }) : [];
const spec = settings ? selections[0] : suppliedSpec;
if (settings && selections.length > 1) {
  // The whole child harness (frontend, service, DB and worker groups) closes
  // before the next spec starts. Never reset production admission/idle guards.
  const base = process.argv.slice(2);
  if (index >= 0) base.splice(index - 2, 2);
  ...
  try {
    for (const selected of selections) {
      ...
      child = spawn(process.execPath, [process.argv[1], ...base, '--spec', selected], { env: process.env, stdio: 'inherit' });
...
async function runFixture() {
  ...
  try {
    ...
    const environment = { ...process.env, ...(settings ? { CMUX_SETTINGS_CYPRESS_SPEC: spec } : {}), CMUX_COMPANION_LOCAL_E2E: '1', CMUX_COMPANION_CYPRESS_BASE_URL: baseUrl,
    ...
    if (!open && (spec || orchestration || settings)) args.push('--spec', spec || (settings ? 'cypress/e2e/settings-onboarding.cy.ts' : 'cypress/e2e/orchestration-core.cy.ts'));
