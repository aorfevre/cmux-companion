...
test("an authenticated issue-card start creates a managed planning workspace with durable issue links", async (t) => {
  ...
  const returnUrl = `/api/worktree-plans/${result.plan.planId}/return-issues`;
  assert.equal((await app.inject({ method: "POST", url: returnUrl })).statusCode, 401);
  assert.equal((await app.inject({ method: "POST", url: returnUrl, headers: { ...headers, origin: "https://other.example" } })).statusCode, 403);
  assert.equal((await app.inject({ method: "POST", url: returnUrl, headers })).statusCode, 400);
  assert.equal((await app.inject({ method: "POST", url: `/api/worktree-plans/${result.plan.planId}/abort`, headers })).statusCode, 200);
  const returned = await app.inject({ method: "POST", url: returnUrl, headers });
  assert.equal(returned.statusCode, 200, returned.body);
  assert.ok(returned.json().issuesReturnedAt);
  assert.equal(worktreeCalls, 1);
  assert.equal(runnerCalls, 1);
  const again = await app.inject({ method: "POST", url: returnUrl, headers });
  assert.equal(again.json().issuesReturnedAt, returned.json().issuesReturnedAt);
  const restarted = await app.inject({ method: "POST", url, headers });
  assert.equal(restarted.statusCode, 200, restarted.body);
  assert.equal(restarted.json().created, true);
