...
describe("contextual mobile features", () => {
  ...
  test("shows CCS quota by account while treating absent windows as unreported", async () => {
    ...
    assert.ok(screen.getByText("Monthly provider limit"));
    ...
    const countdown = within(reportedAccount).getAllByText("Resets in 3d 2h 7m")[0];
    ...
    assert.ok(within(reportedAccount).getAllByText("Resets in 3d 2h 6m")[0]);
    assert.equal(fetchMock.mock.calls.length, fetchesBeforeTick + 1);
