...
**Location:** `server/worktree-dashboard.mjs:515`, `server/worktree-dashboard.mjs:463`, `server/worktree-dashboard.mjs:537`.
...
**Location:** `server/app.mjs:326`, `server/app.mjs:500`, `server/app.mjs:508`, `server/worktree-dashboard.mjs:890`.
...
### PLAT-007 — Medium — maintainability / duplicated parsing and process setup — T6 candidate

**Location:** `server/worktree-inventory.mjs:16`, `server/worktree-dashboard.mjs:862`, `server/worktree-inventory.mjs:13`, `server/repo-catalog.mjs:309`, `server/worktree-operations.mjs:43`.

**Consequence:** Two parsers independently interpret the same NUL-delimited Git worktree stream, and three low-level invocation sites configure Git separately. Fixes to framing or process behavior can diverge across inventory, dashboard and launch safety checks. The current representations differ intentionally; replacing one with the other directly would break callers.

**Evidence:** Both callers request `worktree list --porcelain -z` (`server/worktree-inventory.mjs:71`, `server/worktree-dashboard.mjs:258`). The inventory parser splits double NUL records and produces `primary`, `bare`, boolean `locked`/`prunable` and `lockReason`; dashboard parsing walks fields and produces `detached` plus nullable reason strings/default labels. An executed fixture with a branch record and a detached locked/prunable record confirmed these different shapes. Inventory does not expose detached/prune reason, dashboard does not expose bare/primary, and inventory trims leading field whitespace. There is no shared parser today.

The dashboard **already delegates Git calls to RepoCatalog**. The duplicated low-level execution setup is:

| Site | Existing contract to retain |
| --- | --- |
| `server/worktree-inventory.mjs:13` | Promisified execFile; `git -C`; UTF-8 stdout; 30 s timeout; 16 MiB output; inherited env with optional locks and prompts disabled; original rejection. |
| `server/repo-catalog.mjs:309` | Injected `this.execute`; shared FIFO slot gate and `finally` release; `git -C`; UTF-8; 8 s/1 MiB defaults and truthy timeout/maxBuffer overrides; `process.env`; missing stdout defaults to empty string; original rejection. |
| `server/worktree-operations.mjs:43` | Canonical cwd; dynamically promisified execFile; common-directory lookup; existing implicit execFile defaults; any Git lookup failure calls work without a repository lock; success canonicalizes common dir and locks/rechecks cwd. |

**Action:** T6 should extract a canonical field parser plus compatibility adapters, and a small executor-injected Git runner below RepoCatalog's existing queue. Preserve return shapes, stdout defaults, option precedence, env, original errors and lock behavior. Changing launch fallback, timeouts or parser normalization is not automatically behavior-preserving: characterize before changing. Retain the catalog, dashboard, cleanup and cmux regressions. Add direct parser/runner tests and deterministic local Cypress branch/locked-state coverage in T6; do not claim UI stubs validate raw porcelain. No extraction was performed in T3.
...
- **Rejected: RepoCatalog duplicates JSON persistence.** Its package.json read at `server/repo-catalog.mjs:432` discovers script names; it is not mutable readJson/writeJson state persistence. Archive and favorites do duplicate their small Set persistence implementation (`server/repository-archive.mjs:14`, `server/repository-favorites.mjs:14`), but their consolidation is lower value than the verified failures and is outside T6's Git extraction.
- **Rejected: dashboard contains a third independent Git runner.** It calls the catalog; its `gh` invocation is a different subprocess boundary (`server/worktree-dashboard.mjs:412`). PLAT-007 identifies the actual three Git setup sites.
...
2. **T6 parser boundary:** Cover empty input, final record with/without terminator, repeated separators, LF/CRLF and spaces within paths, branch and detached records, bare primary repositories, marker-only versus reason-bearing locked/prunable fields, whitespace in reasons, unknown fields and adapter-specific defaults. Compare both public adapters and real `git worktree list --porcelain -z` bytes. The one direct existing parser test (`tests/worktree-dashboard.test.mjs:39`) is insufficient to establish this matrix; inventory parsing is exercised indirectly by cleanup fixtures.
3. **T6 runner boundary:** Cover cwd and argument boundaries, default/overridden timeout and maxBuffer, environment inheritance/overrides, injected executor calls, empty/missing stdout, stdout preservation, stderr/nonzero exit/timeout rejection identity, queue release on rejection and launch common-dir fallback. Keep catalog concurrency at the catalog layer. Test option forwarding explicitly rather than assuming the runner may spread arbitrary options.
