...
const tracked = (...patterns) => execFileSync("git", ["ls-files", ...patterns], { encoding: "utf8" }).trim().split("\n").filter(Boolean);
...
let citations = 0;
for (const [i, s] of [...reports, final].entries()) {
  for (const [, p, line] of s.matchAll(/`([^`\s]+):(\d+)`/g)) {
    assert.ok(existsSync(p), `${names[i] || "README.md"}: missing ${p}`);
    const length = read(p).split(/\n/).length - Number(read(p).endsWith("\n"));
    assert.ok(Number(line) >= 1 && Number(line) <= length, `${p}:${line} exceeds ${length}`);
    citations++;
  }
}
console.log(JSON.stringify({ server: coverage.slice(0, 3).map((v) => v.length), client: coverage[3].length, tooling: coverage[4].length, dedicatedGaps: expectedGaps.length, npmSuites: members.length, findings: ids.length, citations, status: "passed" }));
