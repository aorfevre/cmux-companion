...
const routes = ['/?view=sessions', '/?view=launch', '/?view=usage', '/orchestration', '/settings#general', '/settings#dev-repos', '/settings#agents', '/settings#updates', '/settings#advanced'];
describe('Interface consistency matrix', () => {
  for (const width of [360, 390, 1440]) it(`keeps every main surface readable and within ${width}px`, () => {
    ...
    for (const route of routes) {
      ...
      cy.findByRole('link', { name: 'Setup' }).should('have.attr', 'href', '/settings');
      cy.get('main h1').should('be.visible');
      if (route === '/orchestration') { cy.findByRole('button', { name: 'Start a goal' }).should('be.visible'); cy.findByRole('region', { name: 'Goal fleet' }).should('be.visible'); }
  ...
  it('preserves drafts on cancelled navigation, supports keyboard focus and reflows enlarged text', () => {
    ...
    cy.on('window:confirm', () => false); cy.findByRole('button', { name: 'Execution & tools' }).click(); cy.findByLabelText('Default provider').should('have.value', 'codex');
    ...
    cy.visit('/?view=settings'); cy.location('pathname').should('eq', '/settings'); cy.findByRole('button', { name: 'Projects' }).click(); cy.location('hash').should('eq', '#dev-repos'); cy.go('back'); cy.location('hash').should('eq', '');
  });
});
