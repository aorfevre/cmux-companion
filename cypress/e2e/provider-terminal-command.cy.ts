...
describe('Terminal provider commands', () => {
  it('keeps aliases editable and places failed save recovery beside Save on mobile', () => {
    ...
    cy.intercept('POST', '**/api/settings/providers/validate', { ready: true, resolution: { message: 'Resolved xclaude to ccs claude. Permission bypass flags are ignored.' } }).as('validate');
    ...
    cy.contains('Ready for new goals. Resolved xclaude to ccs claude. Permission bypass flags are ignored.').should('exist');
