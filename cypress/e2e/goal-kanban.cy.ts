describe('Goal Kanban navigation', () => {
  it('keeps mobile columns focused and opens the complete goal request', () => {
    ...
    cy.findByRole('navigation', { name: 'Main navigation' }).find('a').should('have.length', 2);
    ...
    cy.findByRole('dialog', { name: 'Goal details' }).should('be.visible').then(dialog => expect(dialog[0].getBoundingClientRect().width).to.equal(390));
    cy.findByRole('dialog', { name: 'Goal details' }).screenshot('goal-panel-mobile');
    cy.findByRole('button', { name: 'Close goal details' }).click();
    cy.contains('.orch-goal-card', 'Outcome 1').should('have.focus');
    cy.get('.orch-kanban').screenshot('goal-kanban-mobile');
    cy.viewport(1200, 900); cy.contains('.orch-goal-card', 'Outcome 0').should('be.visible'); cy.contains('.orch-goal-card', 'Outcome 4').should('be.visible');
    cy.get('.orch-kanban').screenshot('goal-kanban-desktop');
    cy.contains('.orch-goal-card', 'Outcome 1').click();
    cy.findByRole('dialog', { name: 'Goal details' }).then(dialog => expect(dialog[0].getBoundingClientRect().width).to.be.at.least(720));
    cy.findByRole('dialog', { name: 'Goal details' }).screenshot('goal-panel-desktop');
    cy.findByRole('button', { name: 'Close goal details' }).focus(); cy.press(Cypress.Keyboard.Keys.ESC); cy.findByRole('dialog', { name: 'Goal details' }).should('not.exist');
  });
});
