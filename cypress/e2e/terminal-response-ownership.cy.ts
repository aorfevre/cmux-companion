...
for (const staleFailure of [false, true]) it(`keeps terminal B when terminal A's delayed ${staleFailure ? "error" : "replay"} arrives`, () => {
  ...
  cy.intercept("GET", "/api/prompt-queue?*", request => {
    if (request.query.surfaceId !== firstId) { request.reply({ items: [] }); return; }
    queueRequested = true;
    request.alias = "oldQueue";
    return held.then(() => request.reply({ items: [{ id: "old-queue", workspaceId, surfaceId: firstId, text: "STALE A QUEUE", createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(), attempts: 0, lastError: null }] }));
  });
  ...
  cy.wrap(null).should(() => expect(requested && queueRequested).to.equal(true));
