...
settingsSuite('Named Dev repos and unified settings with a real disposable service', () => {
  it('pairs, discovers two directories, selects explicitly, configures a repository and preserves settings', () => {
    ...
    for (const name of ['karven', 'rekord']) {
      cy.findByRole('button', { name: 'Add Dev repo' }).click(); cy.findByLabelText('Dev repo name').type(name);
      cy.task<string>('settingsDevRepoPath', name).then(path => cy.findByLabelText('Directory on this Mac').type(path));
      cy.findByRole('button', { name: 'Validate directory' }).click(); cy.findByRole('button', { name: 'Save Dev repo and discover' }).click();
    ...
    cy.findByRole('button', { name: 'Complete setup' }).click(); cy.contains('Setup complete. You can start a goal.').should('be.visible');
    cy.reload(); cy.findByLabelText('Default provider').should('have.value', 'codex');
    ...
    cy.visit('/orchestration'); cy.findByLabelText('Repository').find('option').should('contain.text', 'karven / My project').and('contain.text', 'rekord / example');
    cy.findByLabelText('Search repositories').type('karven'); cy.findByLabelText('Repository').find('option').should('have.length', 1);
    cy.findByLabelText('What should we accomplish?').type('Plan a disposable change'); cy.findByRole('button', { name: 'Start planning' }).click(); cy.findByRole('navigation', { name: 'Goals' }).should('contain.text', 'Plan a disposable change');
