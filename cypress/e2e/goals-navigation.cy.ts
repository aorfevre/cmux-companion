...
for (const width of [390, 1440]) {
  it(`opens Goals and pairs directly at ${width}px`, () => {
    ...
    cy.visit('/'); cy.findByRole('heading', { name: 'Goals' }).should('be.visible');
    ...
    cy.findByRole('link', { name: 'Set up goals' }).should('be.visible');
    cy.findByRole('navigation', { name: 'Main navigation' }).within(() => { cy.findByRole('link', { name: 'Goals' }).should('have.attr', 'aria-current', 'page'); cy.findByRole('link', { name: 'Sessions' }).should('not.exist'); });
    cy.findByRole('link', { name: 'Browse sessions' }).should('have.attr', 'href', '/?view=sessions');
...
it('recovers from an unavailable Mac without routing through Sessions', () => {
  ...
  cy.findByRole('button', { name: 'Try again' }).click(); cy.findByRole('link', { name: 'Set up goals' }).should('be.visible'); cy.contains('Mac is offline').should('not.exist');
});
